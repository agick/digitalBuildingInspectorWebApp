    ...
    for(let x = 0.0; x <= range; x += increment){
        ...
        for(let y = 0.0; y <= range; y += increment){
            ...
            for(let z = 0.0; z <= range; z += increment){
                lookUpTable[x][y][z] = Math.sqrt(Math.pow(x,2)+Math.pow(y,2)+Math.pow(z,2));
            }
        }
    }
    return lookUpTable;
    ...
    if(precise){
        ...
        if(x > -squareRootMaxRange && x < squareRootMaxRange && 
            ...
            z > -squareRootMaxRange && z < squareRootMaxRange){
                let newX = Math.abs(x.toFixed(1));
                let newY = Math.abs(y.toFixed(1));
                let newZ = Math.abs(z.toFixed(1));
                return squareRootLookupTable[newX][newY][newZ];
            } else {
                return Math.sqrt(Math.pow(x,2)+Math.pow(y,2)+Math.pow(z,2));
            }
    }
...
function generateDistanceField(worldObjectIndex){
    ...
    const layerOneDistanceScale = 254/(distanceFieldBoundingBoxBufferSpace+1);
    const smallestValueAcceptable = 10;
        ...
        for(let x = minX+0.25; x <= maxX; x = x + 0.5){
            for(let y = minY+0.25; y <= maxY; y = y + 0.5){
                for(let z = minZ+0.25; z <= maxZ; z = z + 0.5){
                    ...
                    if(once){
                        distanceFieldTestArray[j] = px0;
                        distanceFieldTestArray[j+1] = py0;
                        distanceFieldTestArray[j+2] = pz0;
                        distanceFieldTestArray[j+3] = px1;
                        distanceFieldTestArray[j+4] = py1;
                        distanceFieldTestArray[j+5] = pz1;
                        distanceFieldTestArray[j+6] = px2;
                        distanceFieldTestArray[j+7] = py2;
                        distanceFieldTestArray[j+8] = pz2;
                        /*console.log("polygon: ",px0,py0,pz0,px1,py1,pz1,px2,py2,pz2);
                        console.log("closestPoint: ",closestPointx,closestPointy,closestPointz);
                        console.log("center cube cords: ",x,y,z);
                        console.log("d: ",d);
                        console.log("t: ",t);
                        console.log("index layer one: ",indexLayerOne);
                        console.log("index layer two: ",indexLayerTwo);*/
                    }
